...
from diffusion.models import get_scheduler_config, get_sd_fine_tuned_model
...
def adv_generator(args, path, i, attack_type, device):
    ...
    if attack_type == "PGD":
        ...
        if args.auto_type == 'plus':
            attack = torchattacks.AutoAttack(model=net, norm='L2', eps=epsilons[i], version='plus', n_classes=10,
                                         seed=2024)
            print("Auto Attack set to Plus")
        elif args.auto_type == 'standard':
            attack = torchattacks.AutoAttack(model=net, norm='L2', eps=epsilons[i], version='standard', n_classes=10, seed=2024)
            print("Auto Attack set to Standard")

        elif args.auto_type == 'rand':
            attack = torchattacks.AutoAttack(model=net, norm='L2', eps=epsilons[i], version='rand', n_classes=10, seed=2024)
...
def main():
    ...
    parser.add_argument('--adv', type=str, default='AUTO', help='type of attack used')
    parser.add_argument('--auto_type', type=str, default='plus', help='version of autoattack')
    parser.add_argument('--net_path', type=str, default='./resnet50.pt', help='path to adv generating net')
    # run args
    ...
    parser.add_argument('--checkpoint_step', default='1000', type=str, help='the checkpoint to load')

    args = parser.parse_args()
    assert len(args.to_keep) == len(args.n_samples)

    # make run output folder
    name = f"FT_Adv_l2_v{args.version}_{args.n_trials}trials_"
    ...
    vae, tokenizer, text_encoder, unet, scheduler = get_sd_fine_tuned_model(args)
    vae = vae.to(device)
    ...
    attack = adv_generator(args, args.net_path, 1, args.adv, device)
    for i in pbar:
        if total > 0:
        ...
        label_adv = torch.tensor(label).to(device).unsqueeze(0)
        img_input = attack(img_input, label_adv)
        with torch.no_grad():
            if args.dtype == 'float16':
